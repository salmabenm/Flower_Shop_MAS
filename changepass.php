...
if (isset($_POST['update_product'])) {
   ...
   if (mysqli_num_rows($select_user) == 0) {
      ...
      // Password conditions
      $password_regex = '/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{4,8}$/';
      if (!preg_match($password_regex, $new_password)) {
         $message[] = 'Password must be between 4 and 8 characters, containing at least one uppercase letter, one lowercase letter, one digit, and one special character.';
      } elseif ($new_password != $confirm_password) {
         $message[] = 'New password and confirm password do not match!';
      } else {
         $new_password = mysqli_real_escape_string($conn, md5($new_password));
