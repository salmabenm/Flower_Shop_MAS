...
if(isset($_POST['update_product'])){
   ...
   mysqli_query($conn, "UPDATE `card` SET name = '$name', price = '$price' WHERE id = '$update_p_id'") or die('query failed');
   ...
   if(!empty($image)){
      if($image_size > 2000000){
         ...
         mysqli_query($conn, "UPDATE `card` SET image = '$image' WHERE id = '$update_p_id'") or die('query failed');
...
<?php
   ...
   $select_products = mysqli_query($conn, "SELECT * FROM `card` WHERE id = '$update_id'") or die('query failed');
...
<form action="" method="post" enctype="multipart/form-data">
   ...
   <a href="admin_card.php" class="btn">go back</a>
</form>
