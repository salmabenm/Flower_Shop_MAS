...
if(isset($_POST['submit'])){
   ...
   $filter_image = filter_var($_POST['image'], FILTER_SANITIZE_STRING);
   $image = mysqli_real_escape_string($conn, $filter_image);
   ...
   if(mysqli_num_rows($select_users) > 0){
      ...
      if($pass != $cpass){
         ...
         mysqli_query($conn, "INSERT INTO `users`(name, email, password,image,phone) VALUES('$name', '$email', '$pass','$image','$phone')") or die('query failed');
...
<section class="form-container">
   ...
   <form action="" method="post">
      ...
      <input type="number" name="phone" class="box" placeholder="enter your number" required>
      <input type="password" name="pass" class="box" placeholder="enter your password" required>
      <input type="password" name="cpass" class="box" placeholder="confirm your password" required>
      <input type="file" accept="image/jpg, image/jpeg, image/png" required class="box" name="image" >
      <input type="submit" class="btn" name="submit" value="register now">
      <p>Already have an account? <a href="login_pep.php">Login now</a></p>
   </form>
